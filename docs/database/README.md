...
- **System and Audit**:
  ...
  - `NotificationLog`: History of notifications sent to users, with `channel` constrained to the backend enum (`EMAIL | PUSH | IN_APP | SYSTEM`) so persisted delivery history stays aligned with the API contract.
