...
export const friendshipService = {
  ...
  sendRequest: async (addresseeId: string) => {
    const { data, error, status } = await api.api.friendships.request.post({ addresseeId }, { headers: getHeaders() })
    if (error) throw new ApiError(status, String(error.value) || 'Failed to send friend request')
    return data
  },

  acceptRequest: async (friendshipId: string) => {
    const { data, error, status } = await api.api.friendships.accept.post({ friendshipId }, { headers: getHeaders() })
  ...
  rejectRequest: async (friendshipId: string) => {
    const { data, error, status } = await api.api.friendships.reject.post({ friendshipId }, { headers: getHeaders() })
    if (error) throw new ApiError(status, String(error.value) || 'Failed to reject friend request')
    return data
  },

  removeFriend: async (friendshipId: string) => {
    const { data, error, status } = await api.api.friendships({ id: friendshipId }).delete(undefined, { headers: getHeaders() })
  ...
  searchUsers: async (query: string): Promise<FriendProfile[]> => {
    const { data, error, status } = await api.api.friendships.search.get({ query: { query }, headers: getHeaders() })
