...
const pendingDayTasks = computed(() => {
  ...
  const dayTasks = tasks.value.filter(t => t.dueDate && new Date(t.dueDate).toDateString() === targetDate && t.status !== 'COMPLETED');
  ...
  if (task.tags && task.tags.length > 0) {
    ...
    if (firstTag && firstTag.color) {
      const colorMap: Record<string, { bg: string, text: string, border: string }> = {
        "bg-red-500": { bg: "bg-red-500/10", text: "text-red-600 dark:text-red-400", border: "border-red-500/20" },
      ...
      return colorMap[firstTag.color] || { bg: "bg-primary/10", text: "text-primary", border: "border-primary/20" };
    }
  }
...
function formatTimeOnly(dateInput: string | Date) {
  return new Intl.DateTimeFormat("es-ES", { hour: "2-digit", minute: "2-digit" }).format(new Date(dateInput));
}
  ...
  <div class="flex flex-col h-[calc(100vh-2rem)] w-full max-w-[1600px] mx-auto gap-4">
    ...
    <div class="flex flex-col xl:flex-row gap-4 flex-1 min-h-0">
      ...
      <div class="w-full xl:w-[400px] flex flex-col gap-4 min-h-0 shrink-0">
        <!-- Summary Card -->
        <Card class="bg-primary text-primary-foreground border-primary shadow-xl rounded-3xl overflow-hidden shrink-0 relative">
          ...
          <CardContent class="p-6 relative z-10 flex flex-col items-start gap-1">
            <p class="font-bold text-primary-foreground/90 uppercase tracking-wider text-sm">
              {{ currentDate.toDateString() === new Date().toDateString() ? 'Hoy tienes' : 'El ' + dfDay.format(currentDate) + ' tienes' }}
            </p>
        ...
        <!-- Upcoming Tasks Card -->
        <Card class="flex-1 flex flex-col min-h-0 border-border shadow-xl rounded-3xl bg-card/60 backdrop-blur-xl">
          <CardHeader class="pb-4 shrink-0 border-b border-border/50">
            <CardTitle class="text-xl font-black flex items-center justify-between">
              Próximas tareas
              <span class="bg-primary/20 text-primary text-sm px-2.5 py-0.5 rounded-full">{{ pendingDayTasks.length }}</span>
            </CardTitle>
          </CardHeader>
          ...
          <CardContent class="p-0 flex-1 flex flex-col min-h-0">
            <ScrollArea class="h-full">
              <div class="p-4 flex flex-col gap-3">
                <div v-if="pendingDayTasks.length === 0" class="text-center text-muted-foreground p-8 flex flex-col items-center gap-2">
                ...
                >
                  <div class="flex items-center gap-3 w-full min-w-0">
                    ...
                    <div class="flex flex-col flex-1 min-w-0">
                      ...
                      <div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground font-medium mt-1">
