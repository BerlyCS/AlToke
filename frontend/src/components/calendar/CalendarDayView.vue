...
function getTaskColor(task: Task) {
  ...
  if (task.priority === "LOW") return "bg-green-500/20 border-green-500 text-green-700 dark:text-green-300";
...
const dayTasksProcessed = computed(() => {
  ...
  processed.forEach((p, idx) => { p.zIndex = 10 + idx });
    ...
    <CardContent class="p-0 flex-1 flex flex-col min-h-0">
      <ScrollArea class="w-full h-full">
        <div class="min-w-[600px] flex flex-col h-full bg-background/50">
          ...
          <!-- Day Header -->
          <div class="grid grid-cols-[60px_1fr] border-b border-border sticky top-0 z-40 bg-card/90 backdrop-blur-md shadow-sm">
            <div class="p-3"></div> <!-- Empty corner -->
            <div class="p-4 text-center border-l border-border/50 flex flex-col items-center justify-center">
                  ...
                  >
                    ...
                    <div class="flex flex-col justify-start min-w-0 h-full">
                      ...
                      <div class="text-[11px] font-bold opacity-75 flex items-center gap-1 mt-1">
                        ...
                        <span v-if="task.estimatedTime" class="ml-1 opacity-70">({{ task.estimatedTime }}m)</span>
                      </div>
                    </div>
                  </div>
                </div>
