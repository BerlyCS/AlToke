...
import { levelUpData, processAchievementsQueue } from '@/composables/useGamification'
...
<template>
  <Dialog v-if="isDesktop" v-model:open="isOpen">
    <DialogContent
      class="sm:max-w-[425px] flex flex-col items-center text-center p-8 border-yellow-500/30 bg-gradient-to-b from-card to-yellow-500/5"
    >
      <DialogHeader class="items-center">
        <div
          class="w-24 h-24 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-full flex items-center justify-center shadow-2xl shadow-yellow-500/30 mb-6 animate-bounce"
        >
          <Trophy class="w-12 h-12 text-yellow-950" />
        </div>
        <DialogTitle
          class="text-3xl font-black text-yellow-600 dark:text-yellow-500 uppercase tracking-widest drop-shadow-sm"
          >¡Nuevo Nivel!</DialogTitle
        >
        <DialogDescription class="text-lg mt-3 font-medium text-muted-foreground">
          Has alcanzado el
          <span class="font-bold text-foreground">Nivel {{ levelUpData.newLevel }}</span
          >.
        </DialogDescription>
      </DialogHeader>
      ...
      <div class="flex flex-col items-center mt-6 w-full">
        ...
        <Button
          size="lg"
          class="w-full mt-6 font-bold text-lg rounded-xl h-12"
          @click="isOpen = false"
        >
          ¡Excelente!
        </Button>
  ...
  <Drawer v-else v-model:open="isOpen">
    <DrawerContent class="border-t-yellow-500/30 bg-gradient-to-b from-card to-yellow-500/5">
      <DrawerHeader class="items-center text-center pt-8">
        <div
          class="w-20 h-20 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-full flex items-center justify-center shadow-2xl shadow-yellow-500/30 mb-4 animate-bounce"
        >
          <Trophy class="w-10 h-10 text-yellow-950" />
        </div>
        <DrawerTitle
          class="text-2xl font-black text-yellow-600 dark:text-yellow-500 uppercase tracking-widest drop-shadow-sm"
          >¡Nuevo Nivel!</DrawerTitle
        >
